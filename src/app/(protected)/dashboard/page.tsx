...
export default async function DashboardPage() {
  ...
  return (
    <DashboardLayout user={session.user}>
      {/** Wrap all dashboard content in a React fragment and pass as children */}
      <>
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-semibold">
              Trabalhos Gerados Recentimente
            </h2>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {generatedWorks
                .slice(0, 3)
                .map(
                  (work: import("@/services/db").GeneratedWork) => (
                    <Card key={work.id}>
                      <CardHeader>
                        <Link href={`/generated/${work.id}`}>
                          <CardTitle className="line-clamp-1">
                            {work.topic
                              ? work.topic
                              : work.instructions
                              ? work.instructions.slice(0, 38) +
                                (work.instructions.length > 38 ? "..." : "")
                              : work.title}
                          </CardTitle>
                        </Link>
                      </CardHeader>
