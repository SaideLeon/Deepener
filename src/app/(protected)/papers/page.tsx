...
export default function PapersPage() {
  ...
  const fetchPapers = async () => {
    try {
      ...
      toast({
        title: "Erro",
        description: "Falha ao carregar os trabalhos.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleDeletePaper = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Sucesso",
        description: "Trabalho excluído com sucesso.",
        variant: "default",
      ...
      toast({
        title: "Erro",
        description: "Falha ao excluir o trabalho.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleGenerateIndex = async () => {
    ...
    try {
      const input: GenerateIndexFromTitlesInput = {
        titles: topicTitles,
        targetLanguage: targetLanguage,
      };
      const result = await generateIndexFromTitles(input);
      setGeneratedIndex(result.generatedIndex);
      setCurrentTextAreaValue(result.generatedIndex || 'As instruções ou estrutura para geração do texto aparecerão aqui...');
      toast({
        title: 'Sucesso',
  ...
  const currentInstructions = activeTab === 'file' ? extractedInstructions : generatedIndex;
            ...
            <Card className="rounded-xl shadow-lg overflow-hidden border-t-4 border-primary/80 bg-card/70 backdrop-blur-md border-border/50 mt-4">
              ...
              <CardContent className="p-8">
                ...
                <Button
                  ...
                  disabled={!topicTitles.trim() || isLoadingIndex}
                  className="mt-8 w-full shadow-md hover:shadow-lg bg-gradient-to-r from-accent to-primary/80 hover:from-accent/90 hover:to-primary/70 text-accent-foreground py-3.5 text-lg font-semibold"
                >
                  {isLoadingIndex ? (
                    <Loader2 className="h-6 w-6 animate-spin mr-2"/>
          ...
          <CardContent className="p-8 space-y-8">
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                ...
                {activeTab === 'titles' && generatedIndex
                  ? 'Índice gerado. Pode ser editado diretamente. Para reprocessar, modifique Títulos/Tema e gere novamente.'
                  : ''}
                {(activeTab === 'file' && !extractedInstructions && !isLoadingExtract) ||
                (activeTab === 'titles' && !generatedIndex && !isLoadingIndex)
            ...
            <div className="mt-6 space-y-4">
              ...
              <MarkdownToDocx
                ...
                fileName={`DeepPenAI_Output_${citationStyle}_${targetLanguage}`}
